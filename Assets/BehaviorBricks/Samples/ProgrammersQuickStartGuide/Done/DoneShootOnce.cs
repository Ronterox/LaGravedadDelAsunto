...
namespace BehaviorBricks.Samples.ProgrammersQuickStartGuide.Done // Programmers Quick Start Guide
{
    ...
    /// it through the Forward axis with the specified velocity.
    /// </summary>
    ...
    public class DoneShootOnce : GOAction
    {
        ...
        /// <remarks>Instantiate the bullet prefab, Search the RigidBody component in bullet instance. We add a rigidBody to bullet 
        ...
        {
            if (!shootPoint) return TaskStatus.FAILED;
            
            // Instantiate the bullet prefab.
            GameObject newBullet = Object.Instantiate(bullet, shootPoint.position, shootPoint.rotation * bullet.transform.rotation);
            
            // Give it a velocity
            Vector3 vel = velocity * shootPoint.forward;
            
            if (newBullet.TryGetComponent(out Rigidbody rigidbody)) rigidbody.velocity = vel;
            else newBullet.AddComponent<Rigidbody>().velocity = vel;
