...
namespace BehaviorBricks.Conditions.System.Perception
{
    /// <summary>
    ...
    public class IsTargetCloseAndInSight : GOCondition
    {
        ...
        /// and if the angle of forward vector with the  raycast direction is lower than the given angle, false otherwase.</returns>
		public override bool Check()
        {
            Vector3 dir = target.transform.position - gameObject.transform.position;
            
            if (dir.sqrMagnitude > closeDistance * closeDistance) return false;
            
            if (Physics.Raycast(gameObject.transform.position + new Vector3(0, 0.1f, 0), dir, out RaycastHit hit)) return hit.collider.gameObject == target && Vector3.Angle(dir, gameObject.transform.forward) < angle * 0.5f;
            return false;
		}
    }
}